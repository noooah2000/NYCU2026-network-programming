...
constexpr uint8_t SOCKS_VERSION = 4;
...
private:
    ...
    std::string shell_response_buf_;
    std::string shell_command_buf_;

    static constexpr int max_length = 4096;
    char data_[max_length];
    std::vector<uint8_t> socks_request_buf_;
    uint8_t socks_reply_buf_[8];

    void do_resolve() {
        auto self(shared_from_this());
        resolver_.async_resolve(sh, sp,
    ...
    void do_request() {
        int port_num = std::stoi(info_.port);
        if (port_num <= 0 || port_num > 65535) {
            output_shell_response(id_, "Invalid port.\n");
            return;
        }
        uint16_t target_port = static_cast<uint16_t>(port_num);

        socks_request_buf_.clear();
        socks_request_buf_.push_back(SOCKS_VERSION);
        socks_request_buf_.push_back(SOCKS_CONNECT);
        socks_request_buf_.push_back(static_cast<uint8_t>(target_port >> 8));
        socks_request_buf_.push_back(static_cast<uint8_t>(target_port & 255));
        socks_request_buf_.insert(socks_request_buf_.end(), {0, 0, 0, 1});
        socks_request_buf_.push_back('x'); // userid
        socks_request_buf_.push_back('\0');
        for (char c : info_.host) {
            socks_request_buf_.push_back(c);
        }
        socks_request_buf_.push_back('\0');

        auto self(shared_from_this());
        boost::asio::async_write(conn_socket_, boost::asio::buffer(socks_request_buf_),
        [this, self](boost::system::error_code ec, std::size_t) {
            if (!ec) {
                do_read_socks_reply();
            } else {
                output_shell_response(id_, "Failed to send SOCKS request.\n");
            }
        });
    }

    void do_read_socks_reply() {
        auto self(shared_from_this());
        boost::asio::async_read(conn_socket_, boost::asio::buffer(socks_reply_buf_, 8),
        [this, self](boost::system::error_code ec, std::size_t) {
            if (!ec) {
                if (socks_reply_buf_[1] == 90) {
                    ...
                    output_shell_response(id_, "SOCKS connection rejected.\n");
                }
            }
            else {
                output_shell_response(id_, "Failed to read SOCKS reply.\n");
            }
        });
    }
    ...
    void do_read() {
        ...
        [this, self](boost::system::error_code ec, std::size_t length){
            if (!ec) {
                ...
                shell_response_buf_.append(data_, length);

                if (shell_response_buf_.find("% ") != std::string::npos) {
                    shell_response_buf_.clear();
                    do_write();
                } 
                else {
                    do_read();
    ...
    void do_write() {
        if (std::getline(file_stream_, shell_command_buf_)) {
            shell_command_buf_ += "\n";
            output_shell_command(id_, shell_command_buf_);
            
            auto self(shared_from_this());
            boost::asio::async_write(conn_socket_, boost::asio::buffer(shell_command_buf_),
            [this, self](boost::system::error_code ec, std::size_t) {
                if (!ec) do_read(); 
            });
        }
